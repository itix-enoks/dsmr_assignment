...
fn test_parse_reorder() {
    let input = fs::read_to_string("examples/good/reorder.dsmr")
        .expect("Failed to read reorder.dsmr file");
    ...
    let expected = vec![
        Telegram::new(
            ...
            TelegramData::Electricity {
                voltages: [
                    TelegramContent::new_value(TelegramContentType::Voltage, (7, 1, Some(1)), Value::Float(241.7), Some(TelegramContentUnit::V)),
                    TelegramContent::new_value(TelegramContentType::Voltage, (7, 1, Some(2)), Value::Float(240.6), Some(TelegramContentUnit::V)),
                    TelegramContent::new_value(TelegramContentType::Voltage, (7, 1, Some(3)), Value::Float(241.92), Some(TelegramContentUnit::V)),
                ],
                currents: [
                    TelegramContent::new_value(TelegramContentType::Current, (7, 2, Some(1)), Value::Float(1.0), Some(TelegramContentUnit::A)),
                    TelegramContent::new_value(TelegramContentType::Current, (7, 2, Some(2)), Value::Float(10.0), Some(TelegramContentUnit::A)),
                    TelegramContent::new_value(TelegramContentType::Current, (7, 2, Some(3)), Value::Float(0.5), Some(TelegramContentUnit::A)),
                ],
                powers: [
                    TelegramContent::new_value(TelegramContentType::Power, (7, 3, Some(1)), Value::Float(1.00), Some(TelegramContentUnit::KW)),
                    TelegramContent::new_value(TelegramContentType::Power, (7, 3, Some(2)), Value::Float(-5.010), Some(TelegramContentUnit::KW)),
                    TelegramContent::new_value(TelegramContentType::Power, (7, 3, Some(3)), Value::Float(2.500), Some(TelegramContentUnit::KW)),
                ],
                total_consumed: TelegramContent::new_value(TelegramContentType::TotalConsumed, (7, 4, Some(1)), Value::Float(11454892.0), Some(TelegramContentUnit::KWH)),
                total_produced: TelegramContent::new_value(TelegramContentType::TotalProduced, (7, 4, Some(2)), Value::Float(1245.0), Some(TelegramContentUnit::KWH)),
            },
        ),
    ];
    ...
    assert_eq!(parsed[0].base.start.telegram_content_type, expected[0].base.start.telegram_content_type);
...
fn test_parse_two_packets() {
    ...
    // Simple voltage test
    if let (TelegramData::Electricity { voltages: v1, .. }, TelegramData::Electricity { voltages: v2, .. }) = (&parsed[0].data, &parsed[1].data) {

        if let (Some(Value::Float(first_voltage)), Some(Value::Float(second_voltage))) = (&v1[0].value, &v2[0].value) {
            assert!((first_voltage - 242.7).abs() < 0.001, "First telegram voltage should be 242.7");
            assert!((second_voltage - 241.7).abs() < 0.001, "Second telegram voltage should be 241.7");
        }
    }
}
...
fn test_parse_good_sequences() {
    ...
    let entries = fs::read_dir(sequences_dir)
        .expect("Failed to read good_sequences directory");
    ...
    for entry in entries {
        ...
        // println!("{:?}", &result);
        assert!(result.is_ok(), "Failed to parse file: {} - Error: {:?}", filename, result.err());

        let telegrams = result.unwrap();
        assert!(!telegrams.is_empty(), "File {} produced no telegrams", filename);

        println!("Successfully parsed {} telegrams from {}", telegrams.len(), filename);
    }
}
